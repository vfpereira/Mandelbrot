...
main = do
	...
	let numIteracoes = 50
        let tamanhoChunks = (length(planoComplexo passoArg) `div` (2^cores))+1
	t0 <- getCurrentTime
	let planoParticao particao = case particao of
					0 -> linhaNormal (planoComplexo passoArg) tamanhoChunks
					1 -> linhaRoundRobin (planoComplexo passoArg) (2^cores) True
					2 -> colunaNormal (planoComplexo passoArg)  (floor telaArg) tamanhoChunks
					_ -> colunaRoundRobin (planoComplexo passoArg)  (floor telaArg) (2^cores) True
	...
	putStrLn $ (show $ (2^cores))  ++ ", " ++ (show $ (totalPontos^2)) ++ ", " ++(show $ tipoParticao particao)  ++ ", " ++ (show $ diffUTCTime t1 t0) ++ ", " ++ (show $ diffUTCTime t2 t1) ++ "\n"
